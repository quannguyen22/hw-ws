 ...
 * @author Alex Snaps
 */
public class TmpOcsCdrHisFileWriterFactory extends CacheWriterFactory {
...
//	private static final String PATH = StringUtils.trimToEmpty( PropsUtil.getInstance().getValue("msisdn.info.1.path"));
	...
	public CacheWriter createCacheWriter(final Ehcache ehcache, final Properties properties) {
		if (StringUtils.equals(Constant.STEP, "2"))
			return new TmpOcsCdrHisFileWriter(new File(Constant.PATH_STEP_2 + "/" + Constant.TMP_OCS_CDR_HIS + ".txt"));
		
		return new TmpOcsCdrHisFileWriter(new File(Constant.PATH_STEP_1 + "/" + Constant.TMP_OCS_CDR_HIS + ".txt"));
	}
}