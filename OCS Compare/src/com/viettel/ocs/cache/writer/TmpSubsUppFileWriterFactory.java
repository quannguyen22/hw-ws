 ...
 * @author Alex Snaps
 */
public class TmpSubsUppFileWriterFactory extends CacheWriterFactory {
...
//	private static final String PATH = StringUtils.trimToEmpty( PropsUtil.getInstance().getValue("msisdn.info.1.path"));
	...
	public CacheWriter createCacheWriter(final Ehcache ehcache, final Properties properties) {
		if (StringUtils.equals(Constant.STEP, "2"))
			return new TmpSubsUppFileWriter(new File(Constant.PATH_STEP_2 + "/" + Constant.TMP_SUBS_UPP + ".txt"));
		
		return new TmpSubsUppFileWriter(new File(Constant.PATH_STEP_1 + "/" + Constant.TMP_SUBS_UPP + ".txt"));
	}
}