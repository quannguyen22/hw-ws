 ...
 * @author Alex Snaps
 */
public class TmpFnNbrFileWriterFactory extends CacheWriterFactory {
...
//	private static final String PATH = StringUtils.trimToEmpty( PropsUtil.getInstance().getValue("msisdn.info.1.path"));
	...
	public CacheWriter createCacheWriter(final Ehcache ehcache, final Properties properties) {
		if (StringUtils.equals(Constant.STEP, "2"))
			return new TmpFnNbrFileWriter(new File(Constant.PATH_STEP_2 + "/" + Constant.TMP_FN_NBR + ".txt"));
		
		return new TmpFnNbrFileWriter(new File(Constant.PATH_STEP_1 + "/" + Constant.TMP_FN_NBR + ".txt"));
	}
}