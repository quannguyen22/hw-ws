...
public class TmpVtCutSeaFileWriter implements CacheWriter {
	private final File file;
	private volatile OutputStream out;
	private volatile boolean write = true;

	private CacheWrapper<String, String> tmpVtCutSea;

	public TmpVtCutSeaFileWriter(final File file) {
		this.file = file;

		// Khoi tao cache de get value tu key
		// Get gia tri tu doi tuong element dang bi sai vi thua chu t o dau dong.
		CacheManager cacheManager = CacheManager.getInstance();
		this.tmpVtCutSea = new EhcacheWrapper<String, String>("tmpVtCutSea", cacheManager);
	}
	...
	public synchronized void write(final Element element) throws CacheException {
		...
		String value = StringUtils.trimToEmpty(tmpVtCutSea.get(key));
