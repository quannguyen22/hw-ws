 ...
 * @author Alex Snaps
 */
public class TmpBlackListSubsFileWriterFactory extends CacheWriterFactory {
...
//	private static final String PATH = StringUtils.trimToEmpty( PropsUtil.getInstance().getValue("msisdn.info.1.path"));
	...
	public CacheWriter createCacheWriter(final Ehcache ehcache, final Properties properties) {
		if (StringUtils.equals(Constant.STEP, "2"))
			return new TmpBlackListSubsFileWriter(new File(Constant.PATH_STEP_2 + "/" + Constant.TMP_BLACKLIST_SUBS + ".txt"));
		
		return new TmpBlackListSubsFileWriter(new File(Constant.PATH_STEP_1 + "/" + Constant.TMP_BLACKLIST_SUBS + ".txt"));
	}
}