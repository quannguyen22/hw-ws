 ...
 * @author Alex Snaps
 */
public class TmpVtCutSeaFileWriterFactory extends CacheWriterFactory {
...
//	private static final String PATH = StringUtils.trimToEmpty( PropsUtil.getInstance().getValue("msisdn.info.1.path"));
	...
	public CacheWriter createCacheWriter(final Ehcache ehcache, final Properties properties) {
		if (StringUtils.equals(Constant.STEP, "2"))
			return new TmpVtCutSeaFileWriter(new File(Constant.PATH_STEP_2 + "/" + Constant.TMP_VT_CUT_SEA + ".txt"));
		
		return new TmpVtCutSeaFileWriter(new File(Constant.PATH_STEP_1 + "/" + Constant.TMP_VT_CUT_SEA + ".txt"));
	}
}